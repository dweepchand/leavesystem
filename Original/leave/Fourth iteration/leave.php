...
if(isset($_GET['type']) && $_GET['type']=='update' && isset($_GET['id'])){
	...
	if ($status == 3 && empty($comment)) {
		...
		mysqli_query($con,"INSERT INTO `leave_approval_history` (leave_id, approver_id, approval_status, comment) VALUES ('$id', '$approved_by', '$status', '$comment')");
								...
								<tbody>
									...
									while($row=mysqli_fetch_assoc($res)){
										...
										while($history_row = mysqli_fetch_assoc($approval_history_res)){
											$approval_history[] = $history_row['approver_name'] . ' (' . ($history_row['approval_status'] == 2 ? 'Approved' : 'Rejected') . '): ' . $history_row['comment'];
										}
...
function update_leave_status(id,select_value){
	...
	if(select_value == 3 && comment == '') {
		alert('Comment is required for rejected leave applications');
		return false;
	}
	window.location.href='leave.php?id='+id+'&type=update&status='+select_value+'&comment='+comment;
}
</script>
<?php
